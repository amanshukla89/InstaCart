...
const checkLowStock = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        if (!product || product.stock === null || product.stock >= Low_stock_threshlod) {
...
// MONTLHY OFFERS EMAIL (1ST OF EVERY MONTH)
...
const autoAssignRider = inngest.createFunction({
    ...
    // wait 5 minutes before attempting next order
    ...
    const result = await step.run("assign-rider", async () => {
        ...
        if (order.deliveryPartnerId) return { skipped: true, reason: "Alrady assigned" };
        if (["Cacelled", "Delivered"].includes(order.status as string)) return { skipped: true, reason: `Order is ${order.status}` };

        // Find an active rider not currently delievering
