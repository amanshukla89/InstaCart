...
const CartSidebar = () => {
    ...
    const { items, removeFromCart, updateQuantity, cartTotal, isCartOpen, setIsCartOpen } =
        useCart()
    ...
    return (
        <>
            {/* Overlay */}
            <div onClick={() => setIsCartOpen(false)} className="fixed 
            inset-0 bg-black/40 z-50 transition-opacity" />
            ...
            shadow-2xl flex flex-col animate-slide-in-right">
                ...
                rounded-xl hover:bg-app-cream transition-colors">
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-4">
                    {items.length === 0 ? (
                        ...
                        h-full text-center">
                            <ShoppingBagIcon className="size-16 text-app-border mb-14" />
                            <h4 className="text-lg font-medium mb-1">Your cart is empty</h4>
                        </div>
                    ) : (
                        items.map((item) => (
                            <div key={item.product._id} className="flex gap-3 bg-app-cream/60 
                            rounded-xl p-3">
                                <img src={item.product.image} alt={item.product.name} className="
                                size-16 rounded-lg object-cover shrink-0"/>
                                    ...
                                    <div className="flex items-center justify-between mt-2">
                                        <div className="flex items-center gap-1.5">
                                            <button onClick={() => updateQuantity(item.product._id, item.quantity - 1)}
                                                className="size-7 rounded-lg bg-white border-app-border flex-center">
                ...
                {items.length > 0 && (
                    <div className="p-5 border-t border-app-border space-y-3">
                        ...
                        <div className="flex justify-between text-sm">
                            ...
                            <span className="font-medium">
                                {deliveryFee === 0 ? <span className="text-app-success">
                                    Free</span> : `${currency}${deliveryFee.toFixed(2)}`}
                            </span>
                        </div>
                        ...
                        <button onClick={() => { setIsCartOpen(false); navgiate('/checkout'); window.scrollTo(0, 0) }}
